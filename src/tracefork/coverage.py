...
#: The three draw kinds `nondet.py` actually records (`RecordingNondet.draws`).
#: Kept in sync with `nondet.py` -- do not invent new kinds here.
DRAW_KINDS: tuple[str, ...] = ("clock", "uuid", "random")
...
def tape_draw_coverage(tape: Tape) -> tuple[dict[str, int], bool, bool]:
    ...
    (no zero-filled kinds), keyed by exactly `nondet.py`'s three kinds.
    """
    draw_counts: dict[str, int] = {}
